...
pub async fn start_background_indexing(
    ...
    // On Android, AI tagging is not available; immediately mark indexing as finished.
    let _ = app_handle.emit("indexing-started", ());
    let _ = app_handle.emit("indexing-finished", ());
    *state.indexing_task_handle.lock().unwrap() = None;
    Ok(())
}
